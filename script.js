/**
 * MusiChris App V66.0 - LOGIN & PLAYBACK ROBUST
...
window.doCreatePlaylist = function() { const name = prompt("Nombre Playlist:"); if(name) { if(!appConfig.data.playlists) appConfig.data.playlists = []; appConfig.data.playlists.push({name:name, songs:[]}); saveData(); updateUI(); } }
...
window.do_create_user_modal = async function() { const name = document.getElementById('newUser_Name').value.trim(); const email = document.getElementById('newUser_Email').value.trim(); const pass = document.getElementById('newUser_Pass').value; if(!name || !email || !pass) return showToast("Faltan datos", 'error'); if(appConfig.data.users.find(u => u.email === email)) return showToast("Existe", 'error'); const avatarUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(name)}`; appConfig.data.users.push({ id: Date.now(), name, email, password: pass, role: 'user', avatar: avatarUrl }); await saveData(); showToast("Registrado", 'success'); closeModal('dom_modal_new_user'); updateUI(); }
...
function updateProgress() { const au = dom.audioElement; if(!au || isNaN(au.duration)) return; const pct = (au.currentTime / au.duration) * 100; if(dom.seekSlider) dom.seekSlider.value = pct; const expSlider = document.getElementById('expandedSeekSlider'); const curTime = document.getElementById('expCurTime'); const totTime = document.getElementById('expTotTime'); if(expSlider) expSlider.value = pct; if(curTime) curTime.textContent = formatTime(au.currentTime); if(totTime) totTime.textContent = formatTime(au.duration); }
...
function showToast(msg, type = 'info') { const t = document.getElementById('customToast'); if(t) { t.textContent = msg; t.className = `customToast show ${type}`; setTimeout(() => t.className = t.className.replace(" show",""), 3000); } }
